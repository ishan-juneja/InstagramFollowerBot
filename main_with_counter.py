	...
	for column in range(1, 4):
		...
		try:
			...
			for element in elements:
				try:
					if element.text[len(element.text) - 5:] == "likes" or element.text[
					                                                      len(element.text) - 6:] == "others":
						like_link = element
				except IndexError:
					pass
		except NoSuchElementException:
		...
		# Following 4 followers from the list
		for counter in range(1, 9):
			try:
				follow_button = driver.find_element(By.XPATH,
				                                    f"/html/body/div[2]/div/div/div[3]/div/div/div[2]/div/div/div[1]/div/div[2]/div/div/div/div/div[2]/div/div[2]/div/div/div[{counter}]/div/div/div/div[3]/div/button/div")
